...
function _appendLog(level, msg) {
  ...
  // INFO fires every 1-3s (the main poll + universal-sync timer) and was
  // appended to disk each time, growing matugen-bridge.log unbounded (59MB+)
  // with constant open/write/flush/close I/O. Keep INFO console-only; only
  // persist WARN/ERROR.
...
// Tab-unload trigger. -1 = uninitialized: the first poll records the file's
// current mtime as a baseline WITHOUT unloading, so a Zen restart never discards
// tabs on its own, only a fresh touch of the trigger file (the ⌥⇧U hotkey) does.
let lastTabUnloadMtime = -1;
...
// Master switch for the Zen Color Boost channel (channel 3). Flips
// enableColorBoost on every domain we've tinted this session so the
// whole-page C++ tint turns off/on live, without deleting the boosts.
// (Session-scoped: a domain tinted in a previous session but not yet
// revisited keeps its stored enableColorBoost until it's synced again.)
function setBoostsEnabled(on) {
  if (!boostsManager) return;
  let n = 0;
  for (const domain of universalBoostedDomains) {
    try {
      ...
      if (!boost) continue;
      boost.boostEntry.boostData.enableColorBoost = on;
      boost.boostEntry.boostData.changeWasMade = true;
      boostsManager.updateBoost(boost);
      n++;
    } catch (e) {
      logError(`setBoostsEnabled[${domain}]: ${e.message}`);
    }
  }
  logInfo(`setBoostsEnabled(${on}): updated ${n} boost(s)`);
}
...
function syncUniversalBoosts() {
  if (!customWebThemeEnabled) return 0; // channel 3 off: don't create/tint boosts
  if (!boostsManager) {
...
function getUserstylesForHostname(hostname) {
  // Master switch (channels 1 & 2): when the custom web theme is off,
  // return a harmless comment. broadcastUserstyles() ships this to open
  // tabs and MatugenChild replaces the injected <style> content with it,
  // clearing BOTH the global wash and per-site overlays live. A non-empty
  // string is required so the existing `if (css)` broadcast/inject guards
  // still fire (an empty string would be skipped, leaving stale CSS).
  if (!customWebThemeEnabled) return "/* matugen web theme disabled */";
  const out = [];
...
function applyJson(data) {
  ...
  if (count > 0) {
    ...
    broadcastToActors(collectValues());
    broadcastUserstyles();
...
function syncWorkspaceTheme(data) {
  ...
  if (!customWebThemeEnabled) return; // channel 3 off: don't re-tint page boosts
  try {
...
// Discard (unload) every tab except the one visible in each window. Discarded
// tabs stay in the tab strip and their content process exits, freeing its RAM;
// they reload automatically when next clicked. Pinned/essentials are included so
// heavy webapps get freed too, change the `tab.selected` guard below to also
// skip `tab.pinned` if you'd rather keep those resident.
function unloadAllTabs() {
  ...
  try {
    ...
    while (wm.hasMoreElements()) {
      ...
      if (!win.gBrowser) continue;
      for (const tab of win.gBrowser.tabs) {
        if (tab.selected) continue;        // never the visible tab
        if (!tab.linkedPanel) continue;    // already discarded → nothing to free
        try {
    ...
    logError(`unloadAllTabs: ${e.message}`);
  }
  logInfo(`Unloaded ${count} tab(s)`);
  return count;
}

function poll() {
  try {
    // Check the tab-unload trigger file (touched by the ⌥⇧U hotkey). On the very
    // first poll we only record a baseline mtime so startup doesn't unload.
    try {
      const tfile = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsIFile);
      const home = Services.dirsvc.get("Home", Ci.nsIFile).path;
      tfile.initWithPath(home + "/.cache/quickshell/zen_tab_unload_trigger");
      if (tfile.exists()) {
        const tm = tfile.lastModifiedTime;
        if (lastTabUnloadMtime === -1) {
          lastTabUnloadMtime = tm;          // baseline; don't unload on startup
        } else if (tm !== lastTabUnloadMtime) {
          lastTabUnloadMtime = tm;
          unloadAllTabs();
        }
      } else if (lastTabUnloadMtime === -1) {
        lastTabUnloadMtime = 0;             // no file yet → baseline set
      }
    } catch (e) {
      logError(`poll tab-unload trigger: ${e.message}`);
    }
      ...
      if (file.exists()) {
        ...
        if (m !== lastWebThemeStateMtime) {
          ...
          if (nextVal !== customWebThemeEnabled) {
            ...
            broadcastUserstyles();                    // channels 1 & 2: clear/restore actor styles
            setBoostsEnabled(customWebThemeEnabled);   // channel 3: toggle Color Boost tint
            if (customWebThemeEnabled) {
              try { syncUniversalBoosts(); } catch (e) {} // re-tint domains visited while off
            }
          }
        }
      } else {
        if (lastWebThemeStateMtime !== 0) {
          ...
          if (!customWebThemeEnabled) {
            ...
            broadcastUserstyles();
            setBoostsEnabled(true);
            try { syncUniversalBoosts(); } catch (e) {}
          }
        }
      }
    } catch (e) {
...
function registerActor() {
  try {
    try { ChromeUtils.unregisterWindowActor(ACTOR_NAME); } catch(e) {}
